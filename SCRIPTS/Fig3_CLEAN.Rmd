...
write.csv(allgenes, "./Scripts/Supplement/Supplemental_Table_4.csv")
```
...
png("./Figures/Fig3/Fig3B_cluster1.png", height = 1250, width = 3000, res = 300, units = "px")
plot_list_grouped[[1]]
dev.off()

pdf("./Figures/Fig3/Fig3B_cluster1.pdf")
plot_list_grouped[[1]]
dev.off()
```
...
png("./Figures/Fig3/Fig3A_heatmap_obese.png", height = 5000, width = 3300, res = 300, units = "px")
...
png("./Figures/Fig3/Fig3A_heatmap_nonobese.png", height = 5000, width = 3300, res = 300, units = "px")
...
pdf("./Figures/Fig3/Fig3A_heatmap_obese.pdf", width = 10, height = 15)
ggplot(data = obese_genes, aes(x = Day, y = Name, fill = mean_expr)) +
  geom_tile() + 
    scale_fill_gradient2(limits = c(-2, 2), name = "Mean\nZ-Score") +
    facet_grid(group ~ Serology, scales = "free") +
...
pdf("./Figures/Fig3/Fig3A_heatmap_nonobese.pdf", width = 10, height = 15)
ggplot(data = nonobese_genes, aes(x = Day, y = Name, fill = mean_expr)) +
  geom_tile() + 
    scale_fill_gradient2(limits = c(-2, 2), name = "Mean\nZ-Score") +
    facet_grid(group ~ Serology, scales = "free") +
...
Old 3A: Heatmap of average scaled counts
```{r}
allgenes <- c(sig_LRT_obese$gene, sig_LRT_nonobese$gene)
...
obese_genes <- meandata %>% filter(group %in% c("Obese", "Both"))
nonobese_genes <- meandata %>% filter(group %in% c("Non Obese", "Both"))

png("./Figures/Fig3/Fig3A_heatmap_obese.png", height = 5000, width = 5000, res = 300, units = "px")
ggplot(data = obese_genes, aes(x = Day, y = Name, fill = mean_expr)) +
  geom_tile() + 
    scale_fill_gradientn(colors = gradient_pal, limits = c(-2, 2), name = "Mean\nZ-Score") +
    facet_grid(group ~ Serology, scales = "free") +
  theme_classic() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size = 18),
          axis.text.y = element_text(size = 20),
...
png("./Figures/Fig3/Fig3A_heatmap_nonobese.png", height = 5000, width = 5000, res = 300, units = "px")
ggplot(data = nonobese_genes, aes(x = Day, y = Name, fill = mean_expr)) +
  geom_tile() + 
    scale_fill_gradientn(colors = gradient_pal, limits = c(-2, 2), name = "Mean\nZ-Score") +
    facet_grid(group ~ Serology, scales = "free") +
...
#HEATMAP of Non-Obese DE Genes
```{r}
genes <- c(sig_LRT_obese$gene, sig_LRT_nonobese$gene)

dds_all <- deseq_import(UGA4_longitudinal_counts_hemo_removed, UGA4_longitudinal_metadata, design_setting = time_serology_model)

ntd_all <- normTransform(dds_all)[genes]

all <- t(data.frame(assay(ntd_all)))

annot_col <- data.frame(genes = colnames(all)) %>%
  mutate(status = ifelse(genes %in% rownames(ntd_obese), "Both", "Nonobese Only"),
         symbol = genenames$hgnc_symbol[match(genes, genenames$ensembl_gene_id)]) %>%
  arrange(status)
...
```{r}
#ntd_nonobese <- normTransform(dds_LRT_time_serology_nonobese)[sig_LRT_nonobese$gene]
#ntd_obese <- normTransform(dds_LRT_time_serology_obese)[sig_LRT_obese$gene]

ntd_obese <- assay(ntd_all[sig_LRT_obese$gene])[,rownames(obese_metadata)]
ntd_nonobese <- assay(ntd_all[sig_LRT_nonobese$gene])[,rownames(nonobese_metadata)]

nonobese <- t(data.frame(ntd_nonobese))

annot_col <- data.frame(genes = colnames(nonobese)) %>%
  mutate(status = ifelse(genes %in% rownames(ntd_obese), "Both", "Nonobese Only"),
         symbol = genenames$hgnc_symbol[match(genes, genenames$ensembl_gene_id)]) %>%
  arrange(status)

rownames(nonobese) <- gsub("X", "", rownames(nonobese))
colnames(nonobese) <- genenames$hgnc_symbol[match(colnames(nonobese), genenames$ensembl_gene_id)]

nonobese <- data.frame(nonobese) %>%
  mutate(BMI = UGA4_longitudinal_metadata$BMI_Bivariate[match(rownames(.), rownames(UGA4_longitudinal_metadata))],
         Response = UGA4_longitudinal_metadata$Serology[match(rownames(.), rownames(UGA4_longitudinal_metadata))],
         Day = UGA4_longitudinal_metadata$Day[match(rownames(.), rownames(UGA4_longitudinal_metadata))]) %>%
  arrange(Day, BMI, Response)

annot_row <- data.frame(Day = nonobese$Day,
                    Response = nonobese$Response)
rownames(annot_row) <- rownames(nonobese)

ann_colors = list(
  Response = c("High Responder" = "#1D6A71", "Non Responder"="#9D5557"),
  Day = c("0" = "#bed3e5", "3" = "#a7c1e0", "7" = "#6f91c3", "28" = "#823f8a"))
...
pheatmap(nonobese[, c(1:36)],annotation_row = annot_row, cluster_rows = FALSE, scale = "column",
         #color = colfunc(100),
         breaks = c(-2, 0, 2),
         show_rownames = FALSE,
         gaps_row = seq(0, 312, 78),
         labels_col = make_bold_names(nonobese[, c(1:36)], colnames, c(annot_col %>% filter(status == "Both") %>% pull(symbol))),
         annotation_colors = ann_colors,
         heatmap_legend_param = list(title = "Scaled Expression"))
```

# HEATMAP of Obese DEGenes
```{r}
obese <- t(data.frame(ntd_obese))

annot_col <- data.frame(genes = colnames(obese)) %>%
  mutate(status = ifelse(genes %in% rownames(ntd_nonobese), "Both", "Obese Only"),
         symbol = genenames$hgnc_symbol[match(genes, genenames$ensembl_gene_id)]) %>%
  arrange(status)

colnames(obese) <- genenames$hgnc_symbol[match(colnames(obese), genenames$ensembl_gene_id)]
rownames(obese) <- gsub("X", "", rownames(obese))

obese <- data.frame(obese) %>%
  mutate(BMI = UGA4_longitudinal_metadata$BMI_Bivariate[match(rownames(.), rownames(UGA4_longitudinal_metadata))],
         Response = UGA4_longitudinal_metadata$Serology[match(rownames(.), rownames(UGA4_longitudinal_metadata))],
         Day = UGA4_longitudinal_metadata$Day[match(rownames(.), rownames(UGA4_longitudinal_metadata))]) %>%
  arrange(Day, BMI, Response)

annot <- data.frame(Day = obese$Day,
                    Response = obese$Response)
rownames(annot) <- rownames(obese)

ann_colors = list(
  Response = c("High Responder" = "#1D6A71", "Non Responder"="#9D5557"),
  Day = c("0" = "#bed3e5", "3" = "#a7c1e0", "7" = "#6f91c3", "28" = "#823f8a"))

pheatmap(obese[, c(1:22)],annotation_row = annot, cluster_rows = FALSE, scale = "column",
         #color = colfunc(100),
         breaks = c(-2, 0, 2),
         show_rownames = FALSE,
         gaps_row = seq(0, 180, 46),
         labels_col = make_bold_names(obese[, c(1:22)], colnames, c(annot_col %>% filter(status == "Both") %>% pull(symbol))),
         annotation_colors = ann_colors,
         heatmap_legend_param = list(title = "Scaled Expression"))
```

```{r}
nrow(UGA4_longitudinal_metadata %>% filter(Day == 0) %>% filter(Serology == "Non Responder"))
```


# Alternative to Fig3B - Heatmaps
```{r}
all_cluster_genes <- c(all_clusters_time_response_continuous[[1]]$genes) #all_clusters_time_response_continuous[[2]]$genes, all_clusters_time_response_continuous[[3]]$genes)

ntd_obese <- assay(normTransform(dds_fullmodel_time_serology_obese)[all_cluster_genes])
ntd_nonobese <- assay(normTransform(dds_fullmodel_time_serology_nonobese)[all_cluster_genes])

all_normalized_clustergenes <- data.frame(t(cbind(ntd_obese, ntd_nonobese))) 

all_normalized_clustergenes <- all_normalized_clustergenes %>%  mutate(Response = UGA4_longitudinal_metadata$Serology[match(rownames(.), rownames(UGA4_longitudinal_metadata))],
         Day = UGA4_longitudinal_metadata$Day[match(rownames(.), rownames(UGA4_longitudinal_metadata))],
         BMI = UGA4_longitudinal_metadata$BMI_Bivariate[match(rownames(.), rownames(UGA4_longitudinal_metadata))]) %>%
  arrange(Response, Day)

  
  
all_normalized_clustergenes_obese <- all_normalized_clustergenes %>% filter(BMI == "Obese")
all_normalized_clustergenes_nonobese <- all_normalized_clustergenes %>% filter(BMI == "Non Obese")

colnames(all_normalized_clustergenes_nonobese) <- genenames$hgnc_symbol[match(colnames(all_normalized_clustergenes_nonobese), genenames$ensembl_gene_id)]
colnames(all_normalized_clustergenes_obese) <- genenames$hgnc_symbol[match(colnames(all_normalized_clustergenes_obese), genenames$ensembl_gene_id)]

annot_obese <- data.frame(Day = all_normalized_clustergenes_obese$Day,
                    Response = all_normalized_clustergenes_obese$Response)
rownames(annot_obese) <- rownames(all_normalized_clustergenes_obese)

annot_nonobese <- data.frame(Day = all_normalized_clustergenes_nonobese$Day,
                    Response = all_normalized_clustergenes_nonobese$Response)
rownames(annot_nonobese) <- rownames(all_normalized_clustergenes_nonobese)
...
png(file = "./Figures/Fig3/Fig3_obese_heatmap.png", height = 1500, width = 3000, res = 300, units = "px")
pheatmap(all_normalized_clustergenes_obese[, c(1:30)],
         annotation_row = annot_obese, 
         cluster_rows = FALSE, 
         scale = "column",
         color = colfunc(100),
         breaks = c(-3, 0, 3),
...
png(file = "./Figures/Fig3/Fig3_nonobese_heatmap.png", height = 1500, width = 3000, res = 300, units = "px")
pheatmap(all_normalized_clustergenes_nonobese[, c(1:30)],
         annotation_row = annot_nonobese, 
         cluster_rows = FALSE, 
         scale = "column",
         color = colfunc(100),
         breaks = c(-3, 0, 3),
...
pdf(file = "./Figures/Fig3/Fig3_obese_heatmap.pdf")
pheatmap(all_normalized_clustergenes_obese[, c(1:30)],
         annotation_row = annot_obese, 
         cluster_rows = FALSE, 
         scale = "column",
...
pdf(file = "./Figures/Fig3/Fig3_nonobese_heatmap.pdf")
pheatmap(all_normalized_clustergenes_nonobese[, c(1:30)],
         annotation_row = annot_nonobese, 
         cluster_rows = FALSE, 
         scale = "column",
